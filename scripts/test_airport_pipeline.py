...
def load_airport_data():
    """Load airport_silo dataset"""
    DATA_DIR = PROJECT_ROOT / 'data' / 'airport_silo'
    datasets = {}
    for f in sorted(DATA_DIR.glob('*.csv')):
        name = f.stem
        datasets[name] = pd.read_csv(f)
        print(f"  Loaded {name}: {len(datasets[name])} rows, {len(datasets[name].columns)} cols")
    return datasets
...
def convert_to_tables_data(datasets, data_dir):
    ...
    for name, df in datasets.items():
        ...
        tables_data[name] = {
            'table_name': name,
            'sample_data': records,  # Use sample_data instead of records
            'columns': columns,
            'row_count': len(df),
            'column_count': len(df.columns),
            'source': str(data_dir / f'{name}.csv'),  # Add source path
        }
    return tables_data
...
def run_pipeline():
    print('='*60, flush=True)
    print('AIRPORT SILO FULL PIPELINE TEST', flush=True)
    print('='*60, flush=True)

    # Load data
    DATA_DIR = PROJECT_ROOT / 'data' / 'airport_silo'
    print('\n[1] Loading airport_silo dataset...', flush=True)
    datasets = load_airport_data()
    ...
    job_id = f'{timestamp}_aviation_airport'
