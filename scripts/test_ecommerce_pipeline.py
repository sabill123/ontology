...
def load_ecommerce_data(data_dir):
    """Load ecommerce_silo dataset"""
    datasets = {}
...
def run_pipeline():
    print('='*60, flush=True)
    print('ECOMMERCE SILO FULL PIPELINE TEST', flush=True)
    print('='*60, flush=True)

    # Load data
    DATA_DIR = PROJECT_ROOT / 'data' / 'ecommerce_silo'
    print('\n[1] Loading ecommerce_silo dataset...', flush=True)
    datasets = load_ecommerce_data(DATA_DIR)
    ...
    job_id = f'{timestamp}_ecommerce_silo'
